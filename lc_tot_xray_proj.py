...
def main(check_filename,redshift_range,obs_bin_rest,obs_bin_received):
    ...
    if os.path.exists(work_path+'/data/'+check_filename+'.npz') is False:
        ...
        flux,pixel,restframe_energy,z_par,z_los = compute_xray(input_filename,vector,radius,redshift_range,PLOT=False, COMPUTE=True,SEL_SPEC_PARTICLE=False)
        flux = flux.T
    ...
    plot_settings = {"data_filter":[1e-41,2e-13],"mode":"log"}
    plot_all(flux_map_contam/pix_a,check_filename+'_contam',r'erg/s/$cm^2/arcmin^2$','plasma',redshift_range,plot_settings)
